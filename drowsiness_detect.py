        ...
        if(eyeAspectRatio < EYE_ASPECT_RATIO_THRESHOLD):
            ...
            if COUNTER >= EYE_ASPECT_RATIO_CONSEC_FRAMES:
                if(flag):
                    ...
                    flag = False   
                if (tk==1):
                    print("Time to start ringing: {}s".format(tic-st))
                    tk=0           
                pygame.mixer.music.play(-1)
                cv2.putText(frame, "You are Drowsy", (150,200), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
                print("Time used: {}s".format(time.time()-tic))
                if (((time.time()-tic) >= 1.5) and ff):
                    ff = 0
                    print("Auto park the car")
                    response = beams_client.publish_to_interests(interests=['hello'],publish_body={'apns': {'aps': {'alert': 'Hello!'}},'fcm': {'notification': {'title': 'Car Alarm','body': 'Driver in danger!!!'}}})
